...
<div class="col-sm-3 w-full "><h2><b>Solicitud de reintegro</b></h2></div>
  ...
  <form action="{{ route('store-reintegro') }}" method="POST" enctype="multipart/form-data">
    @csrf
        ...
        <div>
          ...
          <div class="form-group">
              <div class="space-y-1 text-center">
                <div class="flex text-sm text-gray-600">
                  <label for="file-upload" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-indigo-500">
                    <input id="file-upload"  type="file" class="form-control-file"  name="comprobante" alt="file-name" id="comprobante">
                  </label>
                </div>
              </div>
    ...
    <!-- buttons -->
    <div class="form-group">
            <button class="shadow bg-purple-900 hover:bg-purple-900 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded " >Aceptar</button>
